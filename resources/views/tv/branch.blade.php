...
// Leaderboard — cap at 8
$leaderboard = array_slice($agentLeaderboard ?? [], 0, 8);
@endphp
...
/* Agent row — 2 lines */
.tv-agent{padding:.25rem 0;border-left:3px solid transparent}
...
.tv-agent-rank{font-size:1.4rem;font-weight:800;width:2.2rem;text-align:center;flex-shrink:0}
.tv-agent-name{font-size:1.4rem;font-weight:700;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.tv-agent-pts{font-size:1.4rem;font-weight:800;text-align:right;font-variant-numeric:tabular-nums;flex-shrink:0}
.tv-agent-pts-label{font-size:.9rem;color:#94a3b8;margin-left:.2rem}
.tv-agent-line2{display:flex;align-items:center;gap:.6rem;padding:0 .5rem 0 3.2rem}
.tv-agent-value{font-size:1rem;color:#94a3b8;flex-shrink:0;font-variant-numeric:tabular-nums}
.tv-agent-bar-wrap{flex:1;height:8px;background:#334155;border-radius:999px;overflow:hidden}
...
<div class="tv-screen">
    ...
    {{-- ── ZONE 1: HEADER ── --}}
    <div class="tv-header">
        <div class="tv-branch">{{ $branchName }}</div>
    ...
    <div class="tv-main">
        ...
        <div class="tv-right">
            <div class="tv-lb-title">Agent Leaderboard</div>
            <div class="tv-lb-list">
                ...
                @php
                    ...
                    $agentValueTarget = (float)($agent['points_target'] > 0 ? ($agent['points_target'] / max(1, $pointsTarget)) * $valueTarget : 0);
                @endphp
                <div class="tv-agent {{ $isTop ? 'top' : '' }} {{ $isDimmed ? 'dimmed' : '' }}">
                    <div class="tv-agent-line1">
                        <span class="tv-agent-rank">{{ $medal }}</span>
                        <span class="tv-agent-name">{{ $agent['name'] }}</span>
