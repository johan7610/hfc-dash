...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <div style="background: var(--brand-default, #0b2a4a);" class="rounded-md px-6 py-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <h2 class="text-xl font-bold text-white leading-tight tracking-tight mt-1">Daily Activity Summary</h2>
                <div class="text-sm text-white/60">
            ...
            <form method="GET" action="{{ route('agent.daily.summary') }}" class="flex flex-wrap items-center gap-2">
                <select name="range" class="rounded-md border-0 bg-white/10 text-white text-sm px-3 py-1.5 transition-all duration-300 [&>option]:text-slate-900">
                ...
                @if($range === 'month')
                    <input type="text" name="month" value="{{ $month ?? '' }}" placeholder="YYYY-MM"
                           class="w-28 rounded-md border-0 bg-white/10 text-white text-sm px-3 py-1.5 placeholder:text-white/40 transition-all duration-300" />
                @endif
    ...
    <div class="rounded-md overflow-hidden" style="background: var(--surface); border: 1px solid var(--border);">
        <div class="px-5 py-4" style="border-bottom: 1px solid var(--border);">
            ...
            <div class="text-xs mt-1" style="color: var(--text-muted);">Click an activity name to drill down to your dates list.</div>
        </div>
                    ...
                    @foreach($items as $it)
                        <tr class="transition-all duration-300" style="border-bottom: 1px solid var(--border);"
                            ...
                            <td class="px-4 py-2.5 font-medium">
                                <a class="transition-all duration-300" style="color: var(--brand-icon, #0ea5e9);"
                            ...
                            <td class="px-4 py-2.5 text-right">
                                <span class="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-semibold" style="background: var(--surface-2); color: var(--text-primary);">
                                    {{ (int)$it['count'] }}
                                </span>
                            </td>
