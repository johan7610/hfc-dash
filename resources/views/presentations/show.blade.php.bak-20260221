...
══════════════════════════════════════════════════════════════════════════ --}}
    ...
    <div class="mt-6 bg-white rounded-xl shadow p-5">
        ...
        @else
            ...
            <ul class="space-y-3 mb-4" id="links-list">
                @foreach($links as $link)
                    <li class="border border-gray-100 rounded-lg p-2 text-xs" data-link-id="{{ $link->id }}">
                        {{-- Row 1: Link header --}}
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0 flex items-center gap-1 flex-wrap">
                                ...
                                @endphp
                                <span class="inline-block px-1.5 py-0.5 rounded text-xs font-medium {{ $lExtBadge }}" data-link-badge="{{ $link->id }}">
                                    {{ $lExtLabel }}
                                </span>
                        ...
                        @if($lHasCapture)
                            ...
                            @if($lCapture)
                                ...
                                <div class="mt-1 bg-amber-50 border border-amber-300 rounded px-2 py-1 text-xs text-amber-800 font-medium {{ $lPriceChanges > 0 ? '' : 'hidden' }}" data-price-change="{{ $link->id }}">
                                            ...
                                            @if(!empty($lView['search_summary']))
                                                <dl class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
                                                    @if(!empty($lView['search_summary']['listings_found']))
                                                        ...
                                                        <dd class="text-purple-800 font-medium">{{ $lView['search_summary']['listings_found'] }}</dd>
                                                    @endif
                                                    @if(!empty($lView['search_summary']['total_results']))
                                                        <dt class="text-purple-400 whitespace-nowrap">Total results</dt>
                                                        <dd class="text-purple-800 font-medium">{{ $lView['search_summary']['total_results'] }}</dd>
                                                    @endif
                                                    ...
                                                    @if(!empty($lView['search_summary']['capture_time']))
                                                        ...
                                                        <dd class="text-purple-800">{{ $lView['search_summary']['capture_time'] }}</dd>
                                                    @endif
                                                    @if(!empty($lView['search_summary']['html_bytes']))
                                                        <dt class="text-purple-400 whitespace-nowrap">Page size</dt>
                                                        <dd class="text-purple-800">{{ number_format($lView['search_summary']['html_bytes']) }} bytes</dd>
                                                    @endif
                                                    @if(!empty($lView['search_summary']['parse_status']))
                                                        <dt class="text-purple-400 whitespace-nowrap">Status</dt>
                                                        <dd class="text-purple-800">{{ $lView['search_summary']['parse_status'] }}</dd>
                                                    @endif
                                                </dl>
                                            @endif
                                                    ...
                                                    <tbody>
                                                        @foreach($lView['override_fields'] as $oField)
                                                            <tr class="border-b border-gray-100">
                                                                ...
                                                                <td class="py-1.5 pr-2 {{ $oField['imported'] ? 'text-blue-600' : 'text-gray-300' }}">
        ...
        <form method="POST" action="{{ route('presentations.links.store', $presentation) }}" id="add-link-form" class="space-y-2">
            @csrf
            <div class="flex gap-2">
                        ...
                        @foreach($libraryDocs as $libDoc)
                            <li class="flex items-center justify-between border border-gray-100 rounded-lg px-2 py-1.5">
                                <div class="flex items-center gap-2 min-w-0">
                                    <span class="text-gray-400 shrink-0">&#128206;</span>
                                    <span class="truncate">{{ $libDoc->title ?? $libDoc->original_name }}</span>
                                    <span class="inline-block px-1.5 py-0.5 rounded text-xs font-medium bg-indigo-50 text-indigo-600">
                                        {{ $libDoc->doc_type }}
                                    </span>
                                    <span class="text-gray-400">{{ $libDoc->uploader->name ?? '' }}</span>
                                    <span class="text-gray-400">{{ $libDoc->pivot->created_at ? \Carbon\Carbon::parse($libDoc->pivot->created_at)->format('d M Y') : '' }}</span>
                                </div>
