...
<body>
    <header class="brand-bar">
        ...
        @endif
        <span class="agency-name">{{ $title }}</span>
    </header>

    <main class="legal-content">
        <h1>Privacy Policy</h1>
        {{-- Markdown rendered server-side; operator-authored content trusted at write time. --}}
        {!! $renderedHtml !!}
    </main>

    <footer class="doc-footer">
        @if($lastUpdated)
            Last updated {{ $lastUpdated->format('j F Y') }}
        @endif
        @if($agency && $agency->email)
            · For privacy queries contact <a href="mailto:{{ $agency->email }}" style="color: var(--text-muted);">{{ $agency->email }}</a>
        @endif
    </footer>
</body>
</html>
