...
<form method="POST" action="{{ route('staff-take-on.save-step', [$takeOn, 'leave']) }}">
    ...
    <div class="space-y-4">
        <div class="p-3 text-xs" style="background:rgba(0,212,170,0.04); border:1px solid rgba(0,212,170,0.15); border-radius:3px; color:var(--text-secondary, #6b7280);">
            ...
            @foreach($leaveTypes as $type)
                ...
                <div class="p-4" style="background:var(--surface-2, #f8fafc); border:1px solid var(--border, #e5e7eb); border-radius:3px;">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        @if($type->is_system)
                            <span class="px-1.5 py-0.5 text-[10px] font-bold uppercase" style="background:rgba(148,163,184,0.15); color:#94a3b8; border-radius:3px;">System</span>
                        @endif
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="taken_{{ $type->id }}" step="0.5" min="0" value="0"
                                   class="w-32 px-3 py-2 text-sm focus:outline-none" style="background:#fff; border:1px solid var(--border, #e5e7eb); color:var(--text-primary, #0f172a); border-radius:3px;">
                        </div>
