    ...
    <div x-data="{
        ...
        <div class="flex rounded-lg overflow-hidden" style="border:1px solid var(--border);">
            ...
            <a href="{{ request()->fullUrlWithQuery(['scope' => $val, 'status' => $status]) }}"
               ...
               style="{{ $scope === $val ? 'background:var(--brand-primary,#0b2a4a);color:#fff;' : 'background:var(--surface);color:var(--text-secondary);' }}">
                {{ $lbl }}
            </a>
            @endforeach
        </div>
        ...
        <div class="flex rounded-lg overflow-hidden" style="border:1px solid var(--border);">
            ...
            <a href="{{ request()->fullUrlWithQuery(['status' => $val, 'scope' => $scope, 'agent_id' => $filterAgentId]) }}"
               class="px-3 py-1.5 text-xs font-semibold"
               style="{{ $status === $val ? 'background:var(--brand-primary,#0b2a4a);color:#fff;' : 'background:var(--surface);color:var(--text-secondary);' }}">
                {{ $lbl }}
            </a>
            @endforeach
        </div>
