...
<div class="space-y-5"
    ...
    <div class="flex items-center gap-2 flex-wrap">
        ...
        <span class="text-sm font-semibold" style="color:var(--text-muted);">Core Matches</span>
    </div>

    {{-- Match criteria summary + client view button --}}
        ...
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div class="space-y-3 flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <span class="text-base font-extrabold" style="color:var(--text-primary);">{{ $match->priceRangeLabel() }}</span>
                    @endif
                    @if($match->suburb)
                    <span class="text-xs px-2.5 py-1 rounded-full font-medium" style="background:var(--surface-2); color:var(--text-secondary); border:1px solid var(--border);">
                        📍 {{ $match->suburb }}
                    </span>
                    @endif
                </div>
                <div class="flex flex-wrap gap-x-5 gap-y-2">
                    @if($match->category)
                    <div><span class="text-[10px] font-bold uppercase tracking-wider block" style="color:var(--text-muted);">Category</span><span class="text-xs font-semibold" style="color:var(--text-primary);">{{ $match->category }}</span></div>
                    @endif
                    @if($match->property_type)
                    <div><span class="text-[10px] font-bold uppercase tracking-wider block" style="color:var(--text-muted);">Type</span><span class="text-xs font-semibold" style="color:var(--text-primary);">{{ $match->property_type }}</span></div>
                    @endif
                    @foreach([[$match->beds_min,'Beds'],[$match->baths_min,'Baths'],[$match->garages_min,'Garages'],[$match->parking_min,'Parking']] as [$val,$lbl])
                    @if($val !== null)
                    <div><span class="text-[10px] font-bold uppercase tracking-wider block" style="color:var(--text-muted);">Min {{ $lbl }}</span><span class="text-xs font-semibold" style="color:var(--text-primary);">{{ $val }}+</span></div>
                    @endif
                    @endforeach
                    @if($match->floor_size_min || $match->floor_size_max)
                    <div><span class="text-[10px] font-bold uppercase tracking-wider block" style="color:var(--text-muted);">Floor m²</span><span class="text-xs font-semibold" style="color:var(--text-primary);">{{ $match->floor_size_min ? number_format($match->floor_size_min) : '—' }} – {{ $match->floor_size_max ? number_format($match->floor_size_max) : '—' }}</span></div>
                    @endif
                    @if($match->erf_size_min || $match->erf_size_max)
                    <div><span class="text-[10px] font-bold uppercase tracking-wider block" style="color:var(--text-muted);">Erf m²</span><span class="text-xs font-semibold" style="color:var(--text-primary);">{{ $match->erf_size_min ? number_format($match->erf_size_min) : '—' }} – {{ $match->erf_size_max ? number_format($match->erf_size_max) : '—' }}</span></div>
                    @endif
                </div>
            </div>

            {{-- Right: count + action buttons --}}
            @php $hidden = count($match->hidden_property_ids ?? []); @endphp
            <div class="flex flex-col items-end gap-3 flex-shrink-0">
                <div class="text-right">
                    <div class="text-3xl font-extrabold" style="color:{{ $properties->count() > 0 ? '#00b4d8' : 'var(--text-muted)' }};">
                        {{ $properties->count() }}
                    </div>
                    <div class="text-xs font-semibold" style="color:var(--text-muted);">
                        {{ Str::plural('match', $properties->count()) }} found
                        @if($hidden > 0)
                        <span style="color:var(--text-muted); opacity:.6;">· {{ $hidden }} hidden</span>
                        @endif
                    </div>
                </div>
                ...
                <button type="button"
                        ...
                        onmouseover="this.style.background='rgba(37,211,102,0.20)'" onmouseout="this.style.background='rgba(37,211,102,0.10)'">
                ...
                <a href="{{ $match->sharedUrl() }}" target="_blank"
                   class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold no-underline"
    ...
    <div class="rounded-2xl py-16 text-center" style="background:var(--surface); border:1px solid var(--border);">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="w-12 h-12 mx-auto mb-4 opacity-20" style="color:var(--text-muted);"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 15.803a7.5 7.5 0 0 0 10.607 0Z" /></svg>
        <p class="text-base font-bold" style="color:var(--text-muted);">No active properties match these criteria.</p>
        <p class="text-sm mt-1.5" style="color:var(--text-muted); opacity:.7;">Try broadening the price range, suburb, or room requirements.</p>
        <a href="{{ route('corex.contacts.show', $contact) }}?tab=matches"
    ...
    <div class="space-y-2">
        ...
        @endphp
        <div class="flex items-stretch gap-0 rounded-xl overflow-hidden transition-all duration-200"
             style="background:var(--surface); border:1px solid {{ $isHidden ? 'var(--border)' : 'var(--border)' }}; opacity:{{ $isHidden ? '.55' : '1' }};">

            {{-- Thumbnail --}}
            <div class="flex-shrink-0 relative overflow-hidden" style="width:120px; min-height:90px; background:var(--surface-2);">
                @if($thumb)
                <img src="{{ $thumb }}" alt="{{ $property->title }}" class="absolute inset-0 w-full h-full object-cover">
                @else
                <div class="absolute inset-0 flex items-center justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="w-8 h-8 opacity-20" style="color:var(--text-muted);"><path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Z" /></svg>
                </div>
                @endif
                @if($isHidden)
                <div class="absolute inset-0 flex items-center justify-center" style="background:rgba(0,0,0,0.45);">
            ...
            <div class="flex-1 min-w-0 px-4 py-3 flex flex-col justify-between">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span class="text-[10px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0"
                                  style="background:rgba(239,68,68,0.1); color:#ef4444; border:1px solid rgba(239,68,68,0.2);">Hidden from client</span>
                            @endif
                        </div>
                        <div class="text-sm font-bold mt-1 truncate" style="color:var(--text-primary);">
                            {{ $property->title ?: 'Untitled Property' }}
                        </div>
                        <div class="flex items-center gap-3 mt-1 text-xs flex-wrap" style="color:var(--text-muted);">
                            <span class="font-bold" style="color:#00b4d8;">{{ $property->formattedPrice() }}</span>
                            @if($property->suburb)
                ...
                {{-- Client view counter --}}
                @php $views = $match->propertyViewCount($property->id); @endphp
                <div class="mt-2 pt-2 flex items-center gap-2" style="border-top:1px solid var(--border);">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" style="color:var(--text-muted);"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.641 0-8.58-3.007-9.964-7.178Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /></svg>
                    <span class="text-xs font-semibold" style="color:var(--text-muted);">
                        @if($views > 0)
                            Viewed by client <strong style="color:var(--text-secondary);">{{ $views }} {{ $views === 1 ? 'time' : 'times' }}</strong>
                        @else
            ...
            {{-- Action buttons --}}
            <div class="flex flex-col gap-2 justify-center px-3 py-3 flex-shrink-0" style="border-left:1px solid var(--border);">
                {{-- View in backend --}}
                ...
                {{-- Hide / Unhide from client --}}
                <form method="POST" action="{{ route('corex.contacts.matches.toggleHide', [$contact, $match, $property]) }}">
                    @csrf
                    <button type="submit"
                            class="w-full inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold"
                            style="{{ $isHidden ? 'background:rgba(34,197,94,0.08); color:#16a34a; border:1px solid rgba(34,197,94,0.25);' : 'background:rgba(239,68,68,0.06); color:#ef4444; border:1px solid rgba(239,68,68,0.2);' }}"
    ...
    <div x-show="showWaModal" x-cloak
        ...
        <div class="w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden"
            ...
            <div class="flex items-center justify-between px-6 py-4" style="border-bottom:1px solid var(--border);">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="#25d366">
                            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
                            <path d="M12 0C5.373 0 0 5.373 0 12c0 2.117.554 4.103 1.523 5.824L0 24l6.335-1.509A11.945 11.945 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.854 0-3.6-.483-5.12-1.33l-.368-.214-3.76.896.952-3.656-.238-.384A10.01 10.01 0 0 1 2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/>
                        </svg>
                    </div>
                    <div>
                        ...
                        <div class="text-xs" style="color:var(--text-muted);">{{ $contact->full_name }} · {{ $contact->phone }}</div>
                    </div>
                </div>
            ...
            <div class="px-6 py-5 space-y-3">
                ...
                <p class="text-[10px]" style="color:var(--text-muted);">Opening WhatsApp with this message. The client's link is already included.</p>
            </div>
