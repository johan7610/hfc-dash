    ...
    <div style="background:var(--surface); border:1px solid var(--border); border-radius:6px; overflow:hidden;">
                    ...
                    <a href="{{ route('corex.role-manager') }}"
                       ...
                       style="border:1px solid var(--border);">
                        <div class="w-9 h-9 rounded-md flex items-center justify-center flex-shrink-0" style="background:color-mix(in srgb, var(--brand-icon, #0ea5e9) 12%, transparent);">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="var(--brand-icon, #0ea5e9)" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" /></svg>
                        </div>
                        <div class="flex-1">
        ...
        <div x-show="activeTab === 'feature'" x-cloak class="p-6 space-y-8"
            ...
            <div class="flex gap-2 flex-wrap" style="border-bottom:1px solid var(--border); padding-bottom:16px;">
                @foreach(['documents'=>'Documents','rentals'=>'Rentals','contacts'=>'Contacts','properties'=>'Properties','matches'=>'Matches'] as $fKey => $fLabel)
                <button type="button"
                        @click="featureSection = '{{ $fKey }}'"
                        :style="featureSection === '{{ $fKey }}' ? 'color:var(--brand-icon, #0ea5e9); border-color:var(--brand-icon, #0ea5e9); background:color-mix(in srgb, var(--brand-icon, #0ea5e9) 10%, transparent);' : 'color:var(--text-secondary); border-color:var(--border);'"
                        class="px-4 py-2 rounded-md text-sm font-semibold border transition-all duration-300 outline-none hover:opacity-80">
                    {{ $fLabel }}
                </button>
                @endforeach
            </div>
                    ...
                    <div class="space-y-2 mb-3" x-data="{ showAdd: false, editId: null }">
                        ...
                        <div class="flex items-center justify-between p-3 rounded-md {{ !$rType->is_active ? 'opacity-50' : '' }}"
                            ...
                            <div class="flex items-center gap-3">
                                <button @click="editId = editId === {{ $rType->id }} ? null : {{ $rType->id }}"
                                        class="text-xs hover:opacity-80" style="color:var(--brand-icon, #0ea5e9);">Edit</button>
                                @if(!$rType->is_system)
                            ...
                            <form method="POST" action="{{ route('rental.settings.document-types.update', $rType) }}"
                                ...
                                <div class="w-20">
                                    ...
                                    <input type="color" name="color" value="{{ $rType->color }}"
                                           class="w-full h-8 rounded-md cursor-pointer border"
                                ...
                                <button type="submit" class="corex-btn-primary text-sm px-3 py-1.5">Save</button>
                        ...
                        <div class="mt-2">
                            <button @click="showAdd = !showAdd"
                                    class="text-sm font-medium hover:opacity-80" style="color:var(--brand-icon, #0ea5e9);">+ Add Document Type</button>
                            <div x-show="showAdd" x-cloak class="rounded-md p-3 mt-2"
                                ...
                                <form method="POST" action="{{ route('rental.settings.document-types.store') }}"
                                    ...
                                    <div class="w-20">
                                        <label class="block text-xs mb-1" style="color:var(--text-muted);">Color</label>
                                        <input type="color" name="color" value="#6B7280" class="w-full h-8 rounded-md cursor-pointer border"
                                               style="border-color:var(--border);">
                                    </div>
                                    <label class="flex items-center gap-1.5 text-sm" style="color:var(--text-secondary);">
                                    ...
                                    <button type="submit" class="corex-btn-primary text-sm px-3 py-1.5">Add</button>
                                </form>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-4 rounded-md" style="background:var(--surface-2); border:1px solid var(--border);">
                            <div class="flex items-center justify-between">
                                ...
                                <label class="relative cursor-pointer flex-shrink-0" style="width:44px; height:24px; display:block;">
                                    ...
                                    <input type="checkbox" name="enabled" value="1" x-model="enabled" class="sr-only" {{ $rentalReminderSettings->enabled ? 'checked' : '' }}>
                                    <span class="block w-full h-full rounded-full transition-colors duration-300"
                                          :style="enabled ? 'background:var(--brand-button, #0ea5e9)' : 'background:var(--border-hover)'"></span>
                                    <span class="absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-all duration-300"
                                          :style="enabled ? 'transform:translateX(20px)' : 'transform:translateX(0)'"></span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div x-show="enabled" x-cloak class="space-y-4">
                            <div class="p-4 rounded-md" style="background:var(--surface-2); border:1px solid var(--border);">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <label :style="mode === 'escalating' ? 'border-color:var(--brand-icon, #0ea5e9); background:color-mix(in srgb, var(--brand-icon, #0ea5e9) 8%, transparent);' : 'border-color:var(--border);'"
                                           class="border rounded-md p-3 cursor-pointer transition-all duration-300">
                                    ...
                                    <label :style="mode === 'simple' ? 'border-color:var(--brand-icon, #0ea5e9); background:color-mix(in srgb, var(--brand-icon, #0ea5e9) 8%, transparent);' : 'border-color:var(--border);'"
                                           class="border rounded-md p-3 cursor-pointer transition-all duration-300">
                            ...
                            <div class="p-4 rounded-md space-y-3" style="background:var(--surface-2); border:1px solid var(--border);">
                                ...
                                <div class="flex flex-wrap gap-1 text-xs">
                                    ...
                                    <code class="px-1.5 py-0.5 rounded font-mono" style="background:var(--surface-2); color:var(--text-secondary);">{{ $ph }}</code>
                                    @endforeach
                                </div>
                                <div>
                                ...
                                <div x-show="editCTId !== {{ $cType->id }}"
                                    ...
                                    <div class="flex items-center gap-3">
                                        <button @click="editCTId = {{ $cType->id }}"
                                                class="text-xs font-semibold hover:opacity-80" style="color:var(--brand-icon, #0ea5e9);">Edit</button>
            ...
            <div x-show="featureSection === 'properties'" x-cloak class="space-y-3">
                ...
                <div class="p-4 rounded-md flex items-center justify-between gap-4" style="background:var(--surface-2); border:1px solid var(--border);">
                    ...
                    <form method="POST" action="{{ route('corex.settings.marketing-enabled') }}" class="flex items-center gap-3 flex-shrink-0">
                        ...
                        <span class="text-sm font-semibold" style="color:{{ $marketingEnabled ? 'var(--brand-icon, #0ea5e9)' : 'var(--text-muted)' }};">
            ...
            <div x-show="featureSection === 'matches'" x-cloak class="space-y-5">
                ...
                <div class="p-4 rounded-md flex items-center justify-between gap-4" style="background:var(--surface-2); border:1px solid var(--border);">
                    ...
                    <form method="POST" action="{{ route('corex.settings.matches-enabled') }}" class="flex items-center gap-3 flex-shrink-0">
                        ...
                        <span class="text-sm font-semibold" style="color:{{ $matchesEnabled ? 'var(--brand-icon, #0ea5e9)' : 'var(--text-muted)' }};">
                ...
                <div class="p-4 rounded-md flex items-center justify-between gap-4" style="background:var(--surface-2); border:1px solid var(--border);">
                    ...
                    <form method="POST" action="{{ route('corex.settings.matches-show-on-properties') }}" class="flex items-center gap-3 flex-shrink-0">
                        ...
                        <span class="text-sm font-semibold" style="color:{{ $matchesShowOnProperties ? 'var(--brand-icon, #0ea5e9)' : 'var(--text-muted)' }};">
        ...
        <div x-show="activeTab === 'system'" x-cloak class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="p-4 rounded-md space-y-4" style="background:var(--surface-2); border:1px solid var(--border);">
                    <h3 class="text-xs font-bold uppercase tracking-widest" style="color:var(--text-muted); border-left:3px solid var(--brand-icon, #0ea5e9); padding-left:10px;">General</h3>
                    <div>
                ...
                <div class="space-y-2">
                    <a href="{{ route('admin.p24-suburbs.index') }}"
                       ...
                       style="border:1px solid var(--border);">
                        <div class="w-9 h-9 rounded-md flex items-center justify-center flex-shrink-0" style="background:color-mix(in srgb, var(--brand-icon, #0ea5e9) 12%, transparent);">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="var(--brand-icon, #0ea5e9)" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z" /></svg>
                        </div>
                        <div class="flex-1">
            ...
            {{-- System Information --}}
            <div>
                <h3 class="text-xs font-bold uppercase tracking-widest mb-3" style="color:var(--text-muted); border-left:3px solid var(--brand-icon, #0ea5e9); padding-left:10px;">System Information</h3>
