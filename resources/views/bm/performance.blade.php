        ...
        <div style="background: var(--brand-default, #0b2a4a);" class="rounded-md px-6 py-4">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <form method="GET" action="{{ route('bm.performance') }}" class="flex items-center gap-2">
                        <input type="month" name="period" value="{{ $r['period'] }}" class="h-8 text-sm rounded-md border border-white/20 bg-white/10 text-white px-2 transition-all duration-300" />
            ...
            @if(isset($tvCode) && $tvCode)
                <div class="mt-3 flex items-center gap-3 border-t border-white/10 pt-3">
                    ...
                    <form method="POST" action="{{ route('bm.tv-code.generate') }}" class="inline">
                        @csrf
                        <button type="submit" class="px-2 py-1 text-xs font-semibold rounded-md border border-white/30 text-white hover:bg-white/10 transition-all duration-300">New Code</button>
                    </form>
                    <form method="POST" action="{{ route('bm.tv-code.revoke') }}" class="inline">
                        @csrf
                        <button type="submit" class="px-2 py-1 text-xs font-semibold rounded-md border border-white/30 text-white hover:bg-white/10 transition-all duration-300"
            ...
            @else
                <div class="mt-3 flex items-center gap-3 border-t border-white/10 pt-3">
                    ...
                    <form method="POST" action="{{ route('bm.tv-code.generate') }}" class="inline">
                        @csrf
                        <button type="submit" class="px-2 py-1 text-xs font-semibold rounded-md border border-white/30 text-white hover:bg-white/10 transition-all duration-300">Generate</button>
                    </form>
                </div>
            @endif
        </div>
    ...
    <div class="ds-status-card" style="border-left-color: var(--ds-amber);">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm font-semibold" style="color: var(--ds-amber);">{{ $bmExpiringCount }} mandate{{ $bmExpiringCount === 1 ? '' : 's' }} expiring across your branch in the next 30 days</span>
            </div>
            <a href="{{ route('filing-register.index', ['status' => 'Expiring']) }}" class="text-xs font-semibold underline" style="color: var(--ds-amber);">View</a>
        </div>
    </div>
    @endif
...
<div class="space-y-3">
    ...
    {{-- Set 1: Period (counts) --}}
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <a href="/admin/deals?status=Declined&period={{ $r['period'] }}&branch_id={{ (int)($r['branch_id'] ?? 0) }}" class="block transition-all duration-300">
          ...
          <a href="/admin/deals?status=Pending&period={{ $r['period'] }}&branch_id={{ (int)($r['branch_id'] ?? 0) }}" class="block transition-all duration-300">
          ...
          <a href="/admin/deals?status=Granted&period={{ $r['period'] }}&branch_id={{ (int)($r['branch_id'] ?? 0) }}" class="block transition-all duration-300">
          ...
          <a href="/admin/deals?status=Registered&period={{ $r['period'] }}&branch_id={{ (int)($r['branch_id'] ?? 0) }}" class="block transition-all duration-300">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <a href="/admin/deals?status=Pending&commission_status=Not%20Paid&branch_id={{ (int)($r['branch_id'] ?? 0) }}" class="block transition-all duration-300">
        ...
        <a href="/admin/deals?status=Granted&commission_status=Not%20Paid&branch_id={{ (int)($r['branch_id'] ?? 0) }}" class="block transition-all duration-300">
        ...
        <a href="/admin/deals?status=Registered&commission_status=Not%20Paid&branch_id={{ (int)($r['branch_id'] ?? 0) }}" class="block transition-all duration-300">
  ...
  <div class="ds-status-card">
      <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
          <a href="{{ route('bm.listings', ['filter' => 'active']) }}" class="ds-status-card hover:shadow-md transition-all duration-300 block">
              <div class="ds-label">Active</div>
              <div class="ds-value-lg">{{ (int)($listingStats['total'] ?? 0) }}</div>
          </a>

          <a href="{{ route('bm.listings', ['filter' => 'dom']) }}" class="ds-status-card hover:shadow-md transition-all duration-300 block">
              <div class="ds-label">Avg DOM</div>
              <div class="ds-value-lg">{{ (int)($listingStats['avg_days_on_market'] ?? 0) }}</div>
          </a>

          <a href="{{ route('bm.listings', ['filter' => 'stale']) }}" class="ds-status-card hover:shadow-md transition-all duration-300 block">
              <div class="ds-label">Stale (14d)</div>
              <div class="ds-value-lg">{{ (int)($listingStats['stale'] ?? 0) }}</div>
          </a>

          <a href="{{ route('bm.listings', ['filter' => 'expiring']) }}" class="ds-status-card hover:shadow-md transition-all duration-300 block">
              <div class="ds-label">Expiring (14d)</div>
              <div class="ds-value-lg">{{ (int)($listingStats['expiring_soon'] ?? 0) }}</div>
          </a>

          <a href="{{ route('bm.listings', ['filter' => 'expired']) }}" class="ds-status-card hover:shadow-md transition-all duration-300 block">
...
{{-- Deal Register averages (selected statuses) --}}
        ...
        @if(session("status"))
            <div class="rounded-md p-3 text-sm" style="background: color-mix(in srgb, var(--ds-green) 10%, transparent); border: 1px solid color-mix(in srgb, var(--ds-green) 20%, transparent); color: var(--ds-green);">
                {{ session("status") }}
            </div>
        @endif

        @if($errors->any())
            <div class="rounded-md p-3 text-sm" style="background: color-mix(in srgb, var(--ds-crimson) 10%, transparent); border: 1px solid color-mix(in srgb, var(--ds-crimson) 20%, transparent); color: var(--ds-crimson);">
                {{ implode(", ", $errors->all()) }}
            </div>
        @endif
        ...
        <div class="ds-status-card">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div class="ds-status-card">
                    ...
                    <div class="ds-value-xl leading-tight">
                        {{ $branchDealsActual }}
                        <span class="font-bold" style="color: var(--text-muted);">/ {{ $branchDealsTarget_agentsum }}</span>
                    </div>
                    <div class="mt-2 ds-progress-track">
        ...
        <div class="ds-section-sub mb-3">
            ...
            <span style="color: var(--text-muted);">(Income projection = Agent Value Target Sum × commission rate × company share)</span>
        </div>
        ...
        <div class="ds-status-card">
            <div class="flex items-center justify-between gap-4 flex-wrap">
                ...
                <form method="POST" action="{{ route('bm.performance.save') }}" class="flex items-end gap-2 flex-wrap">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="branch_budget" value="{{ $branchBudget }}"
                               class="w-48 rounded-md text-sm px-3 py-2 transition-all duration-300"
                               style="background: var(--surface-2); border: 1px solid var(--border); color: var(--text-primary);"
                               min="0">
                    </div>
                    <button class="corex-btn-primary px-5 py-2">Save Budget</button>
                </form>
            </div>
            ...
            <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div class="ds-status-card {{ $shortAmount > 0 ? 'ds-status-declined' : 'ds-status-granted' }}">
                    ...
                    @elseif($branchBudget > 0 && $shortAmount > 0)
                        <div class="ds-value-lg" style="color: var(--ds-crimson);">Short by {{ number_format($shortPct, 1) }}%</div>
                        <div class="text-sm mt-1" style="color: var(--text-secondary);">
                            Shortfall: <span class="font-bold">R {{ number_format($shortAmount, 0) }}</span>
                        </div>

                        @if($missingValueTargetsCount > 0)
                            <div class="mt-3 rounded-md p-3" style="border: 1px solid var(--ds-amber); background: color-mix(in srgb, var(--ds-amber) 8%, transparent);">
                                <div class="text-sm font-extrabold" style="color: var(--ds-amber);">Targets incomplete — set missing agent targets</div>
                                    ...
                                    @foreach($missingValueTargets as $u)
                                        <div class="rounded-md p-3 flex items-center justify-between gap-3" style="border: 1px solid var(--border); background: var(--surface);">
                                            ...
                                            <form method="POST" action="{{ route('bm.performance.alignAgentToCompany') }}">
                                                ...
                                                <button class="corex-btn-primary px-4 py-2 whitespace-nowrap">Auto align</button>
                                            </form>
                                        </div>
                                    @endforeach
                                </div>
...
<table class="ds-table min-w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr style="background: var(--surface-2); border-top: 1px solid var(--border);">
                            <td class="px-4 py-3 font-extrabold" style="color: var(--text-primary);">BRANCH TOTAL</td>
