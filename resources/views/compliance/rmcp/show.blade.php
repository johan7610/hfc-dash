...
<div class="-m-4 lg:-m-6">
    <x-page-header title="RMCP v{{ $version->version_number }}" :back-route="route('compliance.rmcp.index')" back-label="RMCP List" :flush="true">
        <x-slot:actions>
            ...
            <span class="inline-flex items-center px-2.5 py-1 text-xs font-semibold" style="background:rgba(0,212,170,0.15); color:#00d4aa; border-radius:3px;">Active</span>
            @elseif($version->status === 'draft')
            <span class="inline-flex items-center px-2.5 py-1 text-xs font-semibold" style="background:rgba(234,179,8,0.15); color:#eab308; border-radius:3px;">Draft</span>
            @else
            <span class="inline-flex items-center px-2.5 py-1 text-xs font-semibold" style="background:rgba(148,163,184,0.15); color:#94a3b8; border-radius:3px;">Superseded</span>
            @endif

            @if($version->canBeEdited())
            @permission('edit_rmcp')
            <a href="{{ route('compliance.rmcp.edit', $version) }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold transition" style="border:1px solid var(--border, #e5e7eb); border-radius:3px; color:var(--text-secondary, #6b7280);">Edit</a>
            @endpermission
            @permission('approve_rmcp')
            <a href="{{ route('compliance.rmcp.approve.form', $version) }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold transition" style="background:#f59e0b; color:#0f172a; border-radius:3px;">Approve</a>
            @endpermission
            @endif

            <a href="{{ route('compliance.rmcp.pdf', $version) }}" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold transition" style="border:1px solid var(--border, #e5e7eb); border-radius:3px; color:var(--text-secondary, #6b7280);">
    ...
    <div class="p-4 lg:p-6">
        ...
        <div class="mb-4 px-4 py-3 text-sm font-semibold" style="background:rgba(234,179,8,0.1); border:1px solid rgba(234,179,8,0.3); border-radius:3px; color:#ca8a04;">
        ...
        @else
        <div class="mb-4 px-4 py-3 text-sm font-semibold" style="background:rgba(148,163,184,0.1); border:1px solid rgba(148,163,184,0.3); border-radius:3px; color:#94a3b8;">
        ...
        <div class="flex gap-6">
            ...
            <div class="hidden lg:block flex-shrink-0" style="width:240px;">
                <div class="sticky top-16">
                    ...
                    <nav class="space-y-0.5" style="max-height:calc(100vh - 120px); overflow-y:auto;">
                        ...
                        <a href="#section-{{ $section->id }}" class="block text-xs py-1 px-2 hover:bg-slate-50 transition" style="color:#64748b; border-radius:3px; line-height:1.4;">
            ...
            <div class="flex-1 min-w-0">
                <div class="bg-white border" style="border-color:var(--border, #e5e7eb); border-radius:3px;">
                    {{-- Cover --}}
                    <div class="text-center" style="background:#0f172a; color:#fff; padding:2.5rem;">
                        <p class="text-xs font-semibold uppercase" style="color:#00d4aa; letter-spacing:2px;">Financial Intelligence Centre Act 38 of 2001</p>
                        <h1 class="text-xl font-bold mt-2" style="font-family:'Plus Jakarta Sans',sans-serif;">{{ $version->title }}</h1>
                    ...
                    <div style="padding:2rem;">
                        ...
                        <div id="section-{{ $section->id }}" class="mb-8">
                            <h2 class="text-base font-bold pb-1.5 mb-3" style="color:#0f172a; border-bottom:2px solid #00d4aa; font-family:'Plus Jakarta Sans',sans-serif;">
                        ...
                        <div class="mt-8 pt-4" style="border-top:2px solid #00d4aa;">
