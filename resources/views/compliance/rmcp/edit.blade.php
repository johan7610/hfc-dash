...
<div class="-m-4 lg:-m-6" x-data="rmcpEditor()">
    <x-page-header title="Edit RMCP v{{ $version->version_number }} Draft" :back-route="route('compliance.rmcp.show', $version)" back-label="View" :flush="true">
        <x-slot:actions>
            ...
            <span class="text-xs" style="color:#00d4aa;" x-show="saved" x-cloak>Saved</span>
        </x-slot:actions>
    </x-page-header>
    ...
    <div class="p-4 lg:p-6">
        ...
        <template x-if="hasUnsaved">
            <div class="mb-4 px-4 py-2 text-sm font-semibold" style="background:rgba(234,179,8,0.1); border:1px solid rgba(234,179,8,0.3); border-radius:3px; color:#ca8a04;">
        ...
        <div class="flex gap-6">
            ...
            <div class="hidden lg:block flex-shrink-0" style="width:200px;">
                <div class="sticky top-16">
                    ...
                    <nav class="space-y-0.5" style="max-height:calc(100vh - 120px); overflow-y:auto;">
                        ...
                        <a href="#edit-section-{{ $section->id }}" class="block text-xs py-1 px-2 hover:bg-slate-50 transition" style="color:#64748b; border-radius:3px;">
            ...
            <div class="flex-1 min-w-0 space-y-4">
                @foreach($version->sections as $section)
                <div id="edit-section-{{ $section->id }}" class="bg-white border p-4" style="border-color:var(--border, #e5e7eb); border-radius:3px;">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <button type="button"
                                ...
                                style="background:#00d4aa; color:#0f172a; border-radius:3px;">
                            Save Section
                        </button>
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" x-ref="title_{{ $section->id }}"
                               value="{{ $section->title }}"
                               @input="markUnsaved()"
                               class="w-full px-3 py-2 text-sm border" style="border-color:var(--border, #e5e7eb); border-radius:3px; font-family:'Plus Jakarta Sans',sans-serif;">
                    </div>
                    <div>
                        ...
                        <textarea x-ref="body_{{ $section->id }}"
                                  @input="markUnsaved()"
                                  rows="10"
                                  class="w-full px-3 py-2 text-sm border font-mono" style="border-color:var(--border, #e5e7eb); border-radius:3px; line-height:1.6;">{{ $section->body_html }}</textarea>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div class="hidden xl:block flex-shrink-0" style="width:240px;">
                <div class="sticky top-16">
                    ...
                    <div class="space-y-1" style="max-height:calc(100vh - 120px); overflow-y:auto;">
                        ...
                        <div class="text-xs p-1.5 bg-slate-50" style="border-radius:3px; cursor:pointer;" @click="navigator.clipboard.writeText('{{ '{{' . $key . '}}' }}')">
