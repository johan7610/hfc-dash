...
<div class="-m-4 lg:-m-6">
    <x-page-header title="Employee Screenings" :back-route="route('compliance.screening.dashboard.index')" back-label="Dashboard" :flush="true">
        <x-slot:actions>
            <a href="{{ route('compliance.screenings.overdue') }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold transition" style="border:1px solid var(--border, #e5e7eb); border-radius:3px; color:var(--text-secondary, #6b7280);">Overdue</a>
    ...
    <div class="p-4 lg:p-6">
        <form method="GET" class="flex items-center gap-3 mb-4">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name..." class="px-3 py-2 text-sm border" style="border-color:var(--border, #e5e7eb); border-radius:3px; max-width:200px; font-family:'Plus Jakarta Sans',sans-serif;">
            ...
            <table class="w-full text-sm" style="font-family:'Plus Jakarta Sans',sans-serif;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid var(--border, #f1f5f9);">
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold" style="border-radius:3px; background:{{ $s->risk_tier === 'high' ? 'rgba(239,68,68,0.15)' : ($s->risk_tier === 'medium' ? 'rgba(234,179,8,0.15)' : 'rgba(0,212,170,0.15)') }}; color:{{ $s->risk_tier === 'high' ? '#ef4444' : ($s->risk_tier === 'medium' ? '#eab308' : '#00d4aa') }};">{{ ucfirst($s->risk_tier) }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            ...
                            <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold" style="background:rgba(0,212,170,0.15); color:#00d4aa; border-radius:3px;">Completed</span>
                            @elseif($s->status === 'flagged')
                            <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold" style="background:rgba(239,68,68,0.15); color:#ef4444; border-radius:3px;">Flagged</span>
                            @else
                            <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold" style="background:rgba(234,179,8,0.15); color:#eab308; border-radius:3px;">{{ $s->completionPercent() }}%</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-xs" style="color:#64748b;">{{ $s->completed_on?->format('d M Y') ?? '-' }}</td>
                        <td class="px-4 py-3 text-xs" style="color:{{ $s->next_due_on && $s->next_due_on->isPast() ? '#ef4444' : '#64748b' }};">{{ $s->next_due_on?->format('d M Y') ?? '-' }}</td>
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('compliance.screenings.show', $s) }}" class="text-xs font-semibold" style="color:#00d4aa;">View</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
