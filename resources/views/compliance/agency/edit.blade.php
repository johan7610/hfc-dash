...
<div class="-m-4 lg:-m-6">
    ...
    <div class="p-4 lg:p-6">
        ...
        @if($errors->any())
        <div class="px-4 py-3 text-sm mb-5" style="background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.25); color:#ef4444; border-radius:3px;">
        ...
        <form id="edit-provision-form" method="POST" action="{{ route('compliance.agency-settings.update', $provision) }}" enctype="multipart/form-data"
              class="max-w-2xl p-6" style="background:var(--surface, #fff); border:1px solid var(--border, #e5e7eb); border-radius:3px;">
            @csrf @method('PATCH')

            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-semibold mb-1" style="color:var(--text-secondary, #6b7280);">Document Type</label>
                    <div class="px-3 py-2 text-sm font-medium" style="background:var(--surface-2, #f8fafc); border:1px solid var(--border, #e5e7eb); color:var(--text-primary, #0f172a); border-radius:3px;">
                ...
                <div>
                    ...
                    <textarea name="notes" rows="2" maxlength="2000"
                              class="w-full px-3 py-2 text-sm" style="background:var(--surface-2, #f8fafc); border:1px solid var(--border, #e5e7eb); color:var(--text-primary, #0f172a); border-radius:3px;">{{ old('notes', $provision->notes) }}</textarea>
                </div>
            </div>
        </form>
    </div>
