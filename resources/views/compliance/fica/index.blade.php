...
<div class="p-6 lg:p-8">
    ...
    <div class="flex gap-1 mb-6 text-sm font-medium border-b border-slate-200">
        ...
        @if($isCO ?? false)
        <a href="{{ route('compliance.fica.index', ['status' => 'agent_approved']) }}" class="px-4 py-2 {{ request('status') === 'agent_approved' ? 'border-b-2 border-indigo-600 text-indigo-700' : 'text-slate-500 hover:text-slate-700' }}">
            Awaiting CO <span class="ml-1 text-xs bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded-full">{{ $counts['agent_approved'] ?? 0 }}</span>
        </a>
        @endif
