...
/**
 * Derives per-agent income totals for a period using the canonical period field.
 *
 ...
 *
 * Used as the "actual" side when comparing Finance Engine rollups to legacy.
 */
class AgentRollupLegacyReader
{
    /**
     * Build a map of user_id => legacy finance totals for a period.
     *
     * Returns:
     *   user_id => [
     *     'agent_income_ex_vat' => float,
     *   ]
     */
    public function buildForPeriod(string $period): array
    {
        ...
        foreach ($rows as $ar) {
            ...
            if (!isset($result[$uid])) {
                $result[$uid] = ['agent_income_ex_vat' => 0.0];
            }
            $result[$uid]['agent_income_ex_vat'] += $agentIncome;
        }

        foreach ($result as &$v) {
            $v['agent_income_ex_vat'] = round($v['agent_income_ex_vat'], 2);
        }
        unset($v);
