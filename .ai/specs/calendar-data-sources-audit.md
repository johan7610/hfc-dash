...
class CalendarVisibilityResolver
{
    ...
    public function canSee(CalendarEvent $event, User $user): bool
    {
        $config = CalendarEventClassSetting::forAgency($user->agency_id)
            ->where('event_class', $event->event_type)
            ->first();

        if (!$config || !$config->is_active) return false;

        $colour = $this->thresholdResolver->resolve(
            $user->agency_id, $event->event_type, $event->event_date
        );
...
### Settings screen: `/corex/settings?tab=feature&fsec=calendar-events`
...
#### FC-13: Computed — FICA Renewal (verified_at + 24 months)

| Field | Value |
|-------|-------|
| Source | `fica_submissions.verified_at` — no explicit "expiry" column stored |
| Computation | FICA verification valid for ~24 months (PPRA standard). Expiry = `verified_at + 24 months` |
| Currently on calendar | **NO** |
...
| **Should be on calendar** | **YES** — computed event |
...
| Note | Needs either a stored `fica_expires_at` column or a computed approach in the calendar source service |
...
### Gap 3: No stored FICA expiry date

**Problem:** FICA verification validity (~24 months) is not stored as an explicit date. `fica_submissions.verified_at` exists but `fica_expires_at` does not.

**Impact:** Calendar integration requires either computing `verified_at + 24 months` on every query or storing an explicit expiry date.

**Recommendation:** Add `fica_expires_at` column to `fica_submissions` or `contacts`. Set via service when FICA is verified. Priority: High (Group A).
...
All values below are **seeder defaults** — agency-configurable via `/corex/settings?tab=feature&fsec=calendar-events`.
...
#### #8 `fica_renewal_due`
```
Source:         Computed: fica_submissions.verified_at + 24 months
                (Needs fica_expires_at column — see Gap Analysis #3)
Priority:       A
...
Notes:          Contact's FICA lapses. Any deal involving this contact is
                non-compliant. Needs new column or compute-on-write.
```
...
4. **FICA renewal** (#8) — deal compliance breach (**needs new column**)
...
1. **On model create/update** — each domain Source Service listens to Eloquent model events (`created`, `updated`) via Laravel observers. When a calendar-relevant date changes, it calls `CalendarEventService::syncFromSource()` which upserts the corresponding `CalendarEvent`.

2. **CalendarEventService::syncFromSource()** — accepts a `PendingCalendarEvent` DTO. Finds existing event by `(source_type, source_id, event_class)`. If the date changed, updates it. If the record was soft-deleted, removes the event. This is idempotent.
...
4. **Nightly reconciliation job** — `command-center:reconcile-calendar` runs at 03:00. Scans all Source Services, compares their output against existing `calendar_events`, and fixes any drift (events that were missed by observers, source dates that changed via raw queries, etc.). This is the safety net, not the primary mechanism.
...
Phase 0 — Infrastructure:
  ...
  app/Services/Calendar/CalendarThresholdResolver.php
  app/Services/Calendar/CalendarVisibilityResolver.php
  app/Services/Calendar/CalendarNotificationDispatcher.php
  ...
  resources/views/command-center/settings/calendar-events.blade.php
  config/corex-permissions.php (add calendar event class permissions)

Phase 1 — Source Services:
  app/Contracts/CalendarSourceContract.php
  app/Services/Calendar/PropertyCalendarSource.php
  app/Services/Calendar/DealCalendarSource.php
  app/Services/Calendar/ComplianceCalendarSource.php
  app/Services/Calendar/PayrollCalendarSource.php
  app/Services/Calendar/RentalCalendarSource.php
  app/Services/Calendar/DocumentCalendarSource.php
  app/Services/Calendar/PeopleCalendarSource.php
  app/Services/Calendar/RecurringCalendarSource.php
...
4. **Command Center reminder defaults already define event categories** that match our `event_class` names (`bond_deadline`, `lease_expiry`, `ffc_expiry`, `mandate_expiry`, `fica_review`, `training_deadline`, `signing_deadline`, `transfer_deadline`) in `resources/views/command-center/settings/index.blade.php`. The threshold system should align with these existing categories.
...
7. **Compliance agent dashboard has a "Notify" button that is disabled** ("Coming soon") — the UI anticipates notification functionality that hasn't been built yet.
