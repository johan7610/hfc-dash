...
- Gate the sidebar item with `@permission('access_{module}')`.
- Gate the route with `->middleware('permission:access_{module}')`.
- Gate action buttons in blade views:
  - Add/Create buttons → `@permission('{module}.create')`
  - Edit buttons → `@permission('{module}.edit')`
  - Delete/Archive buttons → `@permission('{module}.archive')`
- Gate controller actions with `abort_unless(auth()->user()->hasPermission('{module}.action'), 403)`.
- The Role Manager (`/corex/role-manager`) is the single source of truth
  for who can access what. No hardcoded role checks anywhere.
- Use `auth()->user()->hasPermission('key')` — never check role names directly.
...
Data visibility is controlled by the permission system, NOT by role names.
Each module's `{module}.view` permission has a scope value: own, branch, or all.

- **own**: user sees only records they created (WHERE user_id = me)
- **branch**: user sees all records in their branch (WHERE branch_id = my branch)
- **all**: user sees everything across the company

Implementation: Use `PermissionService::getDataScope($user, '{module}')` in
`scopeVisibleTo()` on models. Never check role names for data filtering.

Exceptions (shared data, always visible to all):
...
(global vs branch-specific) — don't override with data scoping.

## Every New Feature Checklist

Before marking ANY feature as done, verify:
- [ ] Navigation link exists (sidebar, menu, or button)
- [ ] Permission keys added (access + view/create/edit/archive)
- [ ] Sidebar gated with `@permission('access_{module}')`
- [ ] Route gated with `->middleware('permission:access_{module}')`
- [ ] Action buttons gated with `@permission('{module}.create/edit/archive')`
- [ ] Controller actions check `hasPermission()`
- [ ] Data scoping via `scopeVisibleTo()` using `getDataScope()`
